...
public class DataSource {
    ...
    public static final String DB_URL = "jdbc:sqlite:E:\\Coding\\SQL\\MusicDataBase\\music.db";
    ...
    public static final String QUERY_ARTIST_FOR_SONG_START = "SELECT " + TABLE_ARTISTS +
    ...
    public static final String QUERY_ALBUMS_BY_ARTIST_START = "SELECT " + TABLE_ALBUMS + "." +
            COLUMN_ALBUM_NAME + " FROM " + TABLE_ALBUMS + " INNER JOIN " + TABLE_ARTISTS + " ON " +
    ...
    public static final String QUERY_VIEW_SONG_INFO_PREP = "SELECT " + COLUMN_ARTISTS_NAME + ", " + COLUMN_SONG_ALBUM +
    ...
    public static final String QUERY_ARTIST = "SELECT " + COLUMN_ARTISTS_ID + " FROM " + TABLE_ARTISTS + " WHERE " + COLUMN_ALBUM_NAME +
            " =? ";
    ...
    public List<Artist> queryArtist(int sortOrder) {
        String sqlQuery = buildSQLQueryArtist(sortOrder);

        try (Statement statement = connection.createStatement(); ResultSet results = statement.executeQuery(sqlQuery)) {
    ...
    private String buildSQLQueryArtist(int sortOrder) {
    ...
    public List<String> queryAlbumsForArtist(String artistName, int sortOrder) {
        String sqlQuery = buildSQLQueryAlbumsForArtist(artistName, sortOrder);

        try (Statement statement = connection.createStatement(); ResultSet results = statement.executeQuery(sqlQuery)) {
            List<String> albums = new ArrayList<>();
            while (results.next()) {
                albums.add(results.getString(1)); //we have only one column in result set
            }
            return albums;
        } catch (SQLException e) {
            printErrorMessage(e);
    ...
    private String buildSQLQueryAlbumsForArtist(String artistName, int sortOrder) {
    ...
    public List<SongArtist> queryArtistsForSong(String songName, int sortOrder) {
        String sqlQuery = buildSQLQueryArtistsForSong(songName, sortOrder);
    ...
    private String buildSQLQueryArtistsForSong(String songName, int sortOrder) {
    ...
    public void createViewForSongArtist() {
    ...
    public void querySongsMetadata() {
        ...
        try (Statement statement = connection.createStatement(); ResultSet resultSet = statement.executeQuery(sql)) {
            ...
            int numberOfColumns = meteData.getColumnCount();

            for (int i = 1; i <= numberOfColumns; i++) {
                System.out.format("Column %d in the song table is names %s\n", i, meteData.getColumnName(i));
            }
        } catch (SQLException e) {
            printErrorMessage(e);
        }
    ...
    private int insertArtist(String name) throws SQLException {
        ...
        if (result.next()) {
            ...
            int affectedRows = insertIntoArtist.executeUpdate(); //returns number of rows
    ...
    private int insertAlbum(String albumName, int artistId) throws SQLException {
        ...
        if (result.next()) {
            ...
            int affectedRows = insertIntoAlbums.executeUpdate(); //returns number of rows
    ...
    public void insertSong(String title, String artist, String album, int track) {
        try {
            ...
            int affectedRows = insertIntoSongs.executeUpdate(); //returns number of rows

            if (affectedRows == 1) {
                connection.commit();
            } else {
                throw new SQLException("The song inset failed");
            }
    ...
    private void printErrorMessage(Exception e) {
        System.out.println("SQLException, something went wrong " + e.getMessage());
        e.printStackTrace();
    }
